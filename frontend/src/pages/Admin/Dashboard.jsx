...
const Dashboard = () => {
  ...
  // Fetch data with retry if needed
  useEffect(() => {
    const fetchData = async () => {
      const success = await getDashboardData();
      if (!success && !fetchAttempted) {
        setFetchAttempted(true);
        setTimeout(() => {
          getDashboardData().then((secondTry) => {
            if (!secondTry) {
      ...
      <div className="card my-5">
        ...
        <div>
          <div className="col-span-3">
            <h2 className="text-xl md:text-2xl bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent font-semibold">
              {greeting}! {user?.name}
            </h2>
            <p className="text-xs md:text-[13px] text-gray-400 tracking-wide mt-1.5">
